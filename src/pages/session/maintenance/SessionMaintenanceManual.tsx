// src/components/session/maintenance/SessionMaintenanceManual.tsx
...
export const SessionMaintenanceManual: React.FC<Props> = ({
  data = [],
  isEdit,
  setCategory,
}) => {
  ...
  return (
    <div className="mt-8">
      <div className="text-gray-400 font-inter grid justify-start mx-2 mt-8 leading-4">
        <span className="font-bold">Manutenção — Manual</span>
      </div>
                      ...
                      <ul className="list-none mt-2 font-inter">
                        {(meta.children || [])
                          ...
                          .map((act: any, aIdx: number) => (
                            <li key={String(act?.key ?? `${pIdx}-${mIdx}-${aIdx}`)} className="my-2 flex gap-2 items-center">
                              <CheckboxSN
